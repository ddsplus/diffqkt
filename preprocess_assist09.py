...
def generate_dataset_files(user_groups, ques_skill_map, output_dir='data/ASSIST09'):
    ...
    # 将问题-技能映射转换为字典以便快速查询
    ques_skill_dict = dict(zip(ques_skill_map['problem_id'], ques_skill_map['skill_id']))
    ...
    for split_name, user_id_list in splits.items():
        ...
        with open(ques_file, 'w') as f_ques, open(skill_file, 'w') as f_skill:
            for user_id in user_id_list:
                ...
                # 获取问题、技能和答案序列
                problem_seq = user_data['problem_id'].tolist()
                skill_seq = [ques_skill_dict.get(p, -1) for p in problem_seq]
                answer_seq = user_data['correct'].tolist()
